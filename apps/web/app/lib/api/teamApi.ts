...
export const getTeams = async (): Promise<Api.TeamResponse[]> => {
  const response =
    await apiClient.get<ApiResponse<Api.TeamResponse[]>>('/teams');
...
export const getTeam = async (id: string): Promise<Api.TeamResponse> => {
  const response = await apiClient.get<ApiResponse<Api.TeamResponse>>(
    `/teams/${id}`
  );
  return response.data.data;
};
...
export const createTeam = async (
  teamData: Api.CreateTeamRequest
): Promise<Api.TeamResponse> => {
  const response = await apiClient.post<ApiResponse<Api.TeamResponse>>(
    '/teams',
    teamData
  );
  return response.data.data;
...
export const updateTeam = async (
  ...
  const response = await apiClient.put<ApiResponse<Api.TeamResponse>>(
    `/teams/${id}`,
    teamData
  );
  return response.data.data;
...
export const getTeamStats = async (
  id: string
): Promise<Api.TeamRosterSummary> => {
  const response = await apiClient.get<ApiResponse<Api.TeamRosterSummary>>(
    `/teams/${id}/stats`
  );
  return response.data.data;
};
