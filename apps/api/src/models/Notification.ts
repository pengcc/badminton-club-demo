...
interface PopulatedNotification
  extends Omit<NotificationAttributes, 'createdBy'> {
  createdBy: {
...
export interface INotification extends NotificationAttributes, BaseDocument {
  populate(
    path: string,
    select?: string
  ): Promise<this & { createdBy: PopulatedNotification['createdBy'] }>;
...
const notificationSchema = new Schema<INotification>(
  {
    title: {
      ...
      minlength: [
        notificationValidationRules.title.minLength,
        'Title too short',
      ],
      maxlength: [
        notificationValidationRules.title.maxLength,
        'Title too long',
      ],
    },
    message: {
      ...
      minlength: [
        notificationValidationRules.message.minLength,
        'Message too short',
      ],
      maxlength: [
        notificationValidationRules.message.maxLength,
        'Message too long',
      ],
    },
    isActive: {
    ...
    startDate: {
      ...
      validate: [
        {
          validator: (v: Date) =>
            notificationValidationRules.startDate.custom?.(v) ?? true,
    ...
    endDate: {
      ...
      validate: [
        {
          validator: (v: Date) =>
            notificationValidationRules.endDate.custom?.(v) ?? true,
 ...
 */
notificationSchema.methods.toView = async function (
  this: INotification
