...
export class PDFService {
  private static readonly TEXTS = {
    de: {
      membershipApplication: {
        ...
        introduction:
          'Hiermit beantrage ich die Mitgliedschaft im Deutsch-Chinesischer Badminton Verein e.V. Die folgenden Angaben sind für die Durchführung des Mitgliedschaftsverhältnisses erforderlich.',
      ...
      sepaMandate: {
        ...
        introduction:
          'Ich ermächtige (Wir ermächtigen) Deutsch-Chinesischer Badminton Verein, Zahlungen von meinem (unserem) Konto mittels Lastschrift einzuziehen. Zugleich weise ich mein (weisen wir unser) Kreditinstitut an, die von Deutsch-Chinesischer Badminton Verein auf mein (unser) Konto gezogenen Lastschriften einzulösen.',
        notice: {
          title: 'Hinweis',
        ...
        paymentFrequency: {
          ...
          annually:
            'jährlich (Bei jährlicher Zahlungsweise reduziert sich der Jahresbeitrag. Siehe Beitragsordnung)',
        },
        applicant: 'Mitgliedschaftsantragsteller',
        accountHolder: 'Vorname und Nachname des Kontoinhabers',
        accountHolderAddress: 'Anschrift des Kontoinhabers',
        signature: {
          date: 'Ort, Datum',
          signature: 'Unterschrift',
        },
   ...
   */
  public static async createMembershipApplicationPDF(
    ...
    try {
      ...
      await this.createApplicationPage(
        pdfDoc,
        data,
        helveticaFont,
        helveticaBold
      ...
      throw new AppError(
        `Failed to create membership application PDF: ${error instanceof Error ? error.message : 'Unknown error'}`,
        500
      );
    }
  }
   ...
   */
  public static async createSEPAMandatePDF(
    input: SepaMandateData
  ): Promise<Buffer> {
    try {
      const pdfDoc = await PDFDocument.create();
      pdfDoc.registerFontkit(fontkit);
      ...
      await this.createSEPAMandatePage(
        pdfDoc,
        input,
        helveticaFont,
        helveticaBold
      ...
      throw new AppError(
        `Failed to create SEPA mandate PDF: ${error instanceof Error ? error.message : 'Unknown error'}`,
        500
      );
    }
  }
  ...
  private static async createApplicationPage(
    ...
    const personalData = [
      ...
      ['Telefon:', data.personalInfo.phone ?? 'Nicht angegeben'],
      ['Geburtsdatum:', dobDisplay],
      [
        'Geschlecht:',
        this.GERMAN_TEXTS.genderMap[data.personalInfo.gender] ||
          data.personalInfo.gender,
      ],
      [
        'Anschrift:',
        `${data.personalInfo.address.street}, ${data.personalInfo.address.postalCode} ${data.personalInfo.address.city}, ${data.personalInfo.address.country}`,
      ],
    ];
    ...
    const membershipType =
      this.GERMAN_TEXTS.membershipTypeMap[data.membershipType] ||
      data.membershipType;
  ...
  private static async createSEPAMandatePage(
    ...
    const { applicantName, bankingInfo } = input;
    ...
    // Default selection
    radioGroup.select(bankingInfo.debitFrequency);

    yPosition -= PAGE_LAYOUT.spacing.paragraph;

    page.drawText(texts.applicant, {
      x: PAGE_LAYOUT.margins.left,
      y: yPosition,
      size: PAGE_LAYOUT.fontSize.body,
      font: helveticaBold,
