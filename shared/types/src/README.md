...
export const matchSchema = z.object({
  ...
  direction: z.nativeEnum(MatchDirection),
  localDate: matchLocalDateSchema,
  localTime: matchLocalTimeSchema,
...
export interface MatchPersistence
  extends Omit<Domain.Match, 'id' | 'teamId' | 'createdAt' | 'updatedAt'>,
          BaseDocument {
...
export interface MatchCardView {
  ...
  opponentName: string;
  direction: MatchDirection;
  localStart: Api.LocalMatchStart;
  result?: Api.MatchResultResponse;
  isUpcoming: boolean;
}
```
...
export class MatchViewTransformer {
  static toCardView(
    ...
    return {
      ...
      result: match.result,
      isUpcoming: new Date(match.startAt).getTime() > Date.now()
    };
  }
}
```
