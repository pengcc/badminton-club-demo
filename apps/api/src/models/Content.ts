...
export interface IContent extends ContentAttributes, BaseDocument {
  populate(
    path: string,
    select?: string
  ): Promise<this & { updatedBy: PopulatedContent['updatedBy'] }>;
 ...
 */
contentSchema.methods.toView = async function (
  this: IContent
): Promise<ContentView> {
...
const localizedTextSchema = new Schema(
  {
    [Language.GERMAN]: { type: String, default: '', trim: true },
    [Language.ENGLISH]: { type: String, default: '', trim: true },
    [Language.CHINESE]: { type: String, default: '', trim: true },
  },
  { _id: false }
);

const homepageContentValuesSchema = new Schema(
  {
    mainMessage: { type: localizedTextSchema, required: true },
    visitUsIntroduction: { type: localizedTextSchema, required: true },
    contactIntroduction: { type: localizedTextSchema, required: true },
  },
  { _id: false }
);

export interface IHomepageContent extends BaseDocument {
...
const homepageContentSchema = new Schema<IHomepageContent>(
  {
    ...
    content: { type: homepageContentValuesSchema, required: true },
    updatedBy: {
      type: Schema.Types.ObjectId,
      ref: 'User',
      required: true,
