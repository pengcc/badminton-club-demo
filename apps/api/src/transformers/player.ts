...
import type { MembershipStatus } from '@club/shared-types/core/enums';
...
export class PlayerPersistenceTransformer {
  ...
  static toDomain(doc: Persistence.PlayerDocument): Domain.Player {
    ...
    if (!Object.values(PlayerType).includes(doc.type)) {
      throw new Error(`PlayerDocument ${doc._id} has invalid type`);
    }
    return {
      id: doc._id.toString(),
...
export class PlayerApiTransformer {
  ...
  static toApi(
    ...
    return {
      ...
      isEffectivelyEligible: options?.isEffectivelyEligible,
  ...
  static fromUpdateRequest(
    req: Api.UpdatePlayerRequest
  ): Partial<Domain.Player> {
    const update: Partial<Domain.Player> = {};

    if (req.singlesRanking !== undefined)
      update.singlesRanking = req.singlesRanking;
    if (req.doublesRanking !== undefined)
      update.doublesRanking = req.doublesRanking;
    if (req.preferredPositions !== undefined)
      update.preferredPositions = req.preferredPositions;
