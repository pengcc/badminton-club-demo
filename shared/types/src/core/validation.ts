 ...
 *    - Example: account-establishment schemas select the allowed User facts
 ...
 *    - Example: form schemas adapt their owning API command schema
 ...
 *     isPlayer: z.boolean()
 *   }).refine(
 *     (data) => !data.isPlayer || data.membershipStatus === 'active',
 ...
 *   userForm: accountEstablishmentSchema.extend({
...
export const SchemaBuilder = {
  ...
  withTimestamp: <T extends z.ZodObject<any>>(schema: T): z.ZodObject<any> =>
    schema.extend(CommonSchemas.timestamp.shape),

  /**
   * Add string ID field
   */
  withStringId: <T extends z.ZodObject<any>>(schema: T): z.ZodObject<any> =>
    schema.extend(CommonSchemas.stringId.shape),
};
